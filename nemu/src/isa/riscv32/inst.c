...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) & ~1u; R(rd) = s->pc + 4);
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I, R(rd) = (word_t)src1 << BITS(imm, 4, 0));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (src2 == 0) ? src1 :  (src1 == 0x80000000 && src2 == 0xFFFFFFFF) ? 0 : (uint32_t)((int32_t)src1 % (int32_t)src2));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (src2 == 0) ? 0xFFFFFFFF : (src1 == 0x80000000 && src2 == 0xFFFFFFFF) ? src1 : (uint32_t)((int32_t)src1 / (int32_t)src2));
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(0xb, s->snpc));
